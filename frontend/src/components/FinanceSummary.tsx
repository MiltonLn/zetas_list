    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: 24 }}>
        <div className="card" style={{ padding: 16, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 600, color: '#ef5350' }}>{formatCurrency(totalExpenses)}</div>
        </div>
        <div className="card" style={{ padding: 16, textAlign: 'center' }}>
          <div style={{ fontSize: 12, opacity: 0.7 }}>Entradas</div>
          <div style={{ fontSize: 20, fontWeight: 600, color: '#66bb6a' }}>{formatCurrency(totalIncome)}</div>
        </div>
        <div className="card" style={{ padding: 16, textAlign: 'center' }}>
          <div style={{ fontSize: 12, opacity: 0.7 }}>Multas Pagadas</div>
          <div style={{ fontSize: 20, fontWeight: 600, color: '#42a5f5' }}>{formatCurrency(totalFinesPaid)}</div>
        </div>
      </div>
    </>
  );
